...
namespace Blazed.Intel.FormatterInternal {
	static class MemorySizes {
		...
		static Info[] GetMemorySizes() {
			...
			ReadOnlySpan<byte>
			bcstToData = new byte[BlazedConstants.MemorySizeEnumCount - (int)BlazedConstants.FirstBroadcastMemorySize] {
			...
			ReadOnlySpan<byte>
			data = new byte[BlazedConstants.MemorySizeEnumCount] {
			...
			var infos = new Info[BlazedConstants.MemorySizeEnumCount];
			for (int i = 0; i < infos.Length; i++) {
				...
				if (i < (int)BlazedConstants.FirstBroadcastMemorySize)
					bcstTo = empty;
				else 					bcstTo = bcstToData[i - (int)BlazedConstants.FirstBroadcastMemorySize] switch {
