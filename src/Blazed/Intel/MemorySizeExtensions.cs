...
namespace Blazed.Intel {
	public static partial class MemorySizeExtensions {
		...
		static MemorySizeInfo[] GetMemorySizeInfos() {
			...
			data = new byte[BlazedConstants.MemorySizeEnumCount * 3] {
			...
			var infos = new MemorySizeInfo[BlazedConstants.MemorySizeEnumCount];
			for (int i = 0, j = 0; i < infos.Length; i++, j += 3) {
				...
				infos[i] = new MemorySizeInfo((MemorySize)i, size, elementSize, elementType, (value & IsSigned) != 0, i >= (int)BlazedConstants.FirstBroadcastMemorySize);
			}
			return infos;
		}
	...
	public readonly struct MemorySizeInfo {
		...
		public MemorySizeInfo(MemorySize memorySize, int size, int elementSize, MemorySize elementType, bool isSigned, bool isBroadcast) {
			...
			if (elementSize > size)
				ThrowHelper.ThrowArgumentOutOfRangeException_elementSize();
			Static.Assert(BlazedConstants.MemorySizeEnumCount <= byte.MaxValue + 1 ? 0 : -1);
